...
def lambda_handler(event: dict[str, Any], context: Any) -> dict[str, Any]:
    ...
    for record in event.get("Records", []):
        try:
            ...
            logger.info(
                f"Processing event: {github_event['type']} from {github_event.get('repo', {}).get('name', 'unknown')}"
            )
            ...
            failed_events.append({"record_id": record.get("messageId"), "error": str(e)})

    logger.info(f"Successfully processed: {len(processed_events)}, Failed: {len(failed_events)}")

    return {
        "statusCode": 200,
        "body": json.dumps({"processed": len(processed_events), "failed": len(failed_events)}),
    }
...
def process_pull_request_event(event: dict[str, Any]) -> dict[str, Any]:
    ...
    return {"pr_action": payload.get("action")}
...
def store_processed_event(processed_event: dict[str, Any]) -> bool:
    ...
    elif event_type == "IssuesEvent" or event_type == "WatchEvent":
