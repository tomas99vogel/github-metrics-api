...
def handle_timeline_visualization_request(query_params: Dict[str, str]) -> Dict[str, Any]:
...
def get_interval_counts(event_types: List[str], start_time: datetime, end_time: datetime) -> Dict[str, int]:
...
def query_interval_count(table, event_type: str, start_time: str, end_time: str) -> int:
    ...
    try:
        response = table.query(
            ...
            KeyConditionExpression=Key('event_type').eq(event_type) & Key('created_at').between(start_time, end_time),
            Select='COUNT'
        )
