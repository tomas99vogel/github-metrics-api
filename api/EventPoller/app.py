...
def lambda_handler(event: Dict[str, Any], context: Any) -> Dict[str, Any]:
    ...
    logger.info(f"GitHub Events polling started: {datetime.now(timezone.utc).isoformat()}")
    ...
    try:
        ...
        if new_events:
            ...
            update_poll_state({
                'etag': result['etag'],
                'last_seen_ids': [event['id'] for event in result['events'][:100]],  # Track last IDs
...
def poll_github_events(poll_state: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        logger.info(
            ...
            datetime.fromtimestamp(int(response.headers.get('x-ratelimit-reset', 0)), timezone.utc).isoformat(),
        )
...
def filter_new_events(events: List[Dict[str, Any]], last_seen_ids: List[str]) -> List[Dict[str, Any]]:
    ...
    logger.info(f"Found {len(new_events)} new events of interest: {[e['type'] for e in new_events]}")
    return new_events
